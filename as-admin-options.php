...
class as_html_theme_layer extends as_html_theme_base {	
	...
	{
		global $as_layers;
		$this->theme_directory = $as_layers['Smatika Theme']['directory'];
		$this->theme_url = $as_layers['Smatika Theme']['urltoroot'];
	...
	function doctype(){
		...
		if ( ($as_request == 'admin/smatika') and (as_get_logged_in_level()>=AS_MEMBER_LEVEL_ADMIN) ) {
		...
		//	Process member actions
			...
			if ( as_clicked( 'doresetoptions' ) ) {
				if ( !as_check_form_security_code( 'admin/smatika', as_post_text( 'code' ) ) )
			...
			$this->content['title']= as_lang_html('admin/admin_title') . ' - ' . as_lang('smatika/smatika_theme');
			...
			$this->content['form'] = array(
				...
				'hidden'  => array(
					'dosaveoptions' => '1',
					'has_js'        => '0',
					'code'          => as_get_form_security_code( 'admin/smatika' ),
				),
			);
			...
			foreach ( $showoptions as $optionname )
				if ( empty( $optionname ) ) {
					...
					$optionfield = array(
						'id'    => $optionname,
						'label' => ( $indented ? '&ndash; ' : '' ) . as_lang( 'smatika/'.$optionname ),
